...
function renderArmorList(){
    ...
    list.innerHTML = ALL_SLOTS.map(slot=>{
        ...
        const rarityBadge = !isEmpty && item.rarity
            ? `<span class="${rClass} font-bold text-[12px] ml-2 font-cinzel">R | ${item.rarity}</span>` : '';
        ...
        if(!isEmpty && item.slots && item.slots.length>0){
            decosHtml=`<div class="flex flex-wrap gap-1.5 mt-2.5">`+item.slots.map((s,idx)=>{
                ...
                return filled
                    ? `<div class="deco-badge deco-filled${s!==filled.s?' deco-diff':''}" title="${filled.n}" onclick="event.stopPropagation();showJewelModal('${filled.id}')">
                           <span class="${s===filled.s ? 'deco-joia-num' : 'deco-slot-dif'}">${s}</span>
                       </div>`
                    : `<div class="deco-badge deco-empty">
                           <span class="deco-slot-empty">${s}</span>
                       </div>`;
            }).join('')+`</div>`;
        }
...
function toggleAccType(){
    ...
    if(type==='decorations'){
        ...
        const txtEl =document.getElementById('title-acc').querySelector('span:last-child');
...
function renderAdminList(){
    ...
    const makeItemHtml=(col,it)=>{
        ...
        const rarBadge = `<span class="${getRarityClass(it.rarity)} font-cinzel font-bold text-[12px]">R | ${it.rarity||1}</span>`;
