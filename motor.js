...
const motionValues = {
    'Espadão':              { mv: 0.52, desc: 'Golpe carregado médio' },
    'Espada e Escudo':      { mv: 0.14, desc: 'Combo rápido (muitos hits)' },
    'Martelo':              { mv: 0.52, desc: 'Golpe forte / combo spin' },
    'Lança':                { mv: 0.23, desc: 'Toque triplo médio' },
    'Transmachado':         { mv: 0.45, desc: 'Modo espada / Descarga' },
    'Glaive Inseto':        { mv: 0.30, desc: 'Aéreo + combo padrão' },
    'Fuzilarco Pesado':     { mv: 0.20, desc: 'Disparo Normal Nv3' },
    'Espada Longa':         { mv: 0.39, desc: 'Corte médio + Spirit Blade' },
    'Duplas-Lâminas':       { mv: 0.12, desc: 'Combo rápido (muitos hits)' },
    'Berrante de Caça':     { mv: 0.42, desc: 'Slam + melodias' },
    'Lançarma':             { mv: 0.35, desc: 'Poke + Full Burst' },
    'Lâmina Dínamo':        { mv: 0.78, desc: 'Modo Espada / golpe forte' },
    'Fuzilarco Leve':       { mv: 0.10, desc: 'Disparo Rápido (muitos hits)' },
    'Arco':                 { mv: 0.18, desc: 'Tiro Dragão Nv3 × tiros' },
};
...
function renderJewelStock(sortBy){
    ...
    if(!container) return;
    const sorted=[...DB.decorations].sort((a,b)=>{
        if(sortBy==='name') return a.n.localeCompare(b.n);
        return b.s-a.s||a.n.localeCompare(b.n); // level desc, then name
    });
...
function updateAllStats(sharpnessOverride){
    ...
    let weaponElementHtml='', weaponSharpnessHtml='', weaponSharpnessBase2=null;
    ...
    // ── Gerar barra de afiação AGORA que finalAtk está calculado ──
    ...
    // ── DANO REAL ESTIMADO (Motion Value) ─────────────────────────
    ...
    const mv = mvData ? mvData.mv : null;
    // Zona de Impacto média padrão: 70 (parte fraca comum — cabeça/pescoço)
    const hitzoneNormal = 70;
    const hitzoneWeak   = 80;
    // Dano Real = Ataque × Afiação × MV × Hitzone/100
    const atkWithSharp = Math.round(finalAtk * sharpMult);
    const realDmgBase     = mv ? Math.round(atkWithSharp * mv * hitzoneNormal/100) : null;
    const realDmgWeak     = mv ? Math.round(atkWithSharp * mv * hitzoneWeak/100) : null;
    const realDmgCrit     = mv&&finalAff>0 ? Math.round(atkWithSharp*(1+critDmgBonus/100)*mv*hitzoneNormal/100) : null;
    const realDmgWeakCrit = mv&&bonusDmgWeakspot ? Math.round(atkWithSharp*(1+critDmgBonus/100)*mv*hitzoneWeak/100*(1+bonusDmgWeakspot/100)) : null;
...
document.getElementById('offense-display').innerHTML=`
        ...
        <div class="stat-card rounded shadow-md cursor-pointer hover:border-[#e8d5a3] transition" onclick="toggleBreakdown('breakdown-scenarios')">
            ...
            <div class="stat-breakdown hidden mt-2 space-y-1" id="breakdown-scenarios">
                ...
                ${finalAtkWeakspot>finalAtkSharp ? `<div class="flex justify-between"><span class="text-[#c9b07a]">Ponto Fraco:</span><span class="text-[#e3c421] font-cinzel font-bold">${finalAtkWeakspot}</span></div>` : ''}
                ${finalAtkCritWeak ? `<div class="flex justify-between"><span class="text-[#c9b07a]">Crítico | Ponto Fraco:</span><span class="text-[#f97316] font-cinzel font-bold">${finalAtkCritWeak}</span></div>` : ''}
        ...
        <!-- Dano Real por golpe (Motion Value) -->
        ${mvData ? `
        <div class="stat-card rounded shadow-md cursor-pointer hover:border-[#e8d5a3] transition" onclick="toggleBreakdown('breakdown-real-dmg')">
            <div class="flex justify-between items-center">
                ...
                <span class="text-[#e3c421] font-cinzel font-bold text-[20px] leading-none">${realDmgBase}</span>
            </div>
            <div class="stat-breakdown hidden mt-2 space-y-1" id="breakdown-real-dmg">
                <div class="text-[12px] text-[#6b5e43] italic mb-1">${wType} · MV ${(mv*100).toFixed(0)}% · Zona de Impacto ${hitzoneNormal}</div>
                ${bd('Normal (Zona de Impacto '+hitzoneNormal+')', realDmgBase, 'text-[#e3c421]')}
                ${realDmgWeak ? bd('Ponto Fraco (Zona '+hitzoneWeak+')', realDmgWeak, 'text-[#e3c421]') : ''}
                ${realDmgCrit&&finalAff>0 ? bd('Crítico', realDmgCrit, 'text-[#4090f0]') : ''}
                ${realDmgWeakCrit ? bd('Crítico | Ponto Fraco', realDmgWeakCrit, 'text-[#f97316]') : ''}
                <div class="text-[11px] text-[#4a412f] italic mt-1 border-t border-[#3a3020] pt-1">Estimativa com hitzone média. Varia por parte do monstro.</div>
            </div>
        </div>` : ''}
        <!-- Afinidade -->
        <div class="stat-card cursor-pointer hover:border-[#e8d5a3] transition rounded shadow-md" onclick="toggleBreakdown('breakdown-aff')">
            ...
            <div class="stat-breakdown hidden" id="breakdown-aff">
                ...
                ${bonusAffWeakspot ? `<div class="flex justify-between mt-1 pt-1 border-t border-[#3a3020]"><span class="text-[#c9b07a]">Ponto Fraco (afin.):</span><span class="text-[#e3c421]">+${bonusAffWeakspot}% → ${affWeakspot}%</span></div>` : ''}
                ${bonusDmgWeakspot ? `<div class="flex justify-between"><span class="text-[#c9b07a]">Ponto Fraco (dano):</span><span class="text-[#e3c421]">+${bonusDmgWeakspot}%</span></div>` : ''}
