...
window.toggleBreakdown = function(id){
    ...
    document.querySelectorAll('.stat-breakdown:not(.hidden)').forEach(b=>{ if(b.id!==id) b.classList.add('hidden'); });
...
function openEditor(slot){
    ...
    <div class="flex flex-col mb-4">
        ...
        ${!isEmpty&&item.rarity?`<span class="inline-block mt-2 text-[13px] font-bold font-cinzel uppercase tracking-wide ${getRarityClass(item.rarity)}">Raridade ${item.rarity}</span>`:''}
    </div>
    ...
    if(isEmpty){
        ...
        if(slot!=='weapon'&&!isEmpty){
            ...
            if(sks.length) info+=`<div class="mt-5 pt-4 border-t border-[#3a3020] space-y-3">${sks.map(sk=>`<div class="flex justify-between items-baseline gap-2"><span class="text-[#a89060] text-[15px] font-crimson leading-none">${sk}</span><span class="text-[#6a5e40] font-cinzel text-[13px] flex-shrink-0">Nv ${item.skills[sk]}</span></div>`).join('')}</div>`;
        }
...
function updateAllStats(){
    ...
    // Multiplicador de ataque por nível de afiação (valores do jogo MHW)
    const sharpnessMultTable = {
      'Nenhuma':1.00,'Vermelha':0.50,'Laranja':0.75,'Amarela':1.00,
      'Verde':1.05,'Azul':1.20,'Branca':1.32,'Roxa':1.39
    };
    const weaponSharpness = build['weapon']&&build['weapon'].data&&build['weapon'].data.sharpness!=='Nenhuma'
        ? build['weapon'].data.sharpness : null;
    const sharpMult = weaponSharpness ? (sharpnessMultTable[weaponSharpness]||1) : 1;
    const finalAtkSharp = weaponSharpness ? Math.round(finalAtk * sharpMult) : finalAtk;
...
document.getElementById('offense-display').innerHTML=`
        <div class="stat-card cursor-pointer hover:border-[#e8d5a3] transition rounded shadow-md" onclick="toggleBreakdown('breakdown-atk')">
            <div class="flex justify-between items-center">
                <span class="stat-label">Ataque${weaponSharpness?' <span class=\"text-[12px] font-crimson normal-case font-normal text-[#8a7a56]\">(c/ afiação)</span>':''}</span>
            ...
            <div class="stat-breakdown hidden" id="breakdown-atk">
                ${bd('Arma (base)', atkBase)}
                ${weaponSharpness ? bd('Afiação ('+weaponSharpness+')', '×'+sharpMult.toFixed(2)) : ''}
                ${bdBonus('Reforço de Ataque', bonusAtk)}
                ${bdBonus('Agitador', bonusAtkFromAgit)}
            </div>
        </div>
        ${weaponSharpnessHtml}
        ${finalAtkWeakspot > finalAtkSharp ? `
        <div class="stat-card cursor-pointer hover:border-[#e8d5a3] transition rounded shadow-md" onclick="toggleBreakdown('breakdown-weakspot')">
            <div class="flex justify-between items-center">
                <span class="stat-label">P. Feridas</span>
                <span class="text-[#60d060] font-cinzel font-bold text-[20px] leading-none">${finalAtkWeakspot}</span>
            </div>
            <div class="stat-breakdown hidden" id="breakdown-weakspot">
                ${bd('Base', finalAtkSharp)}
                ${bdBonus('P. Fraca (afin.)', bonusAffWeakspot, '%')}
                ${bdBonus('P. Ferida (dano)', bonusDmgWeakspot, '%')}
            </div>
        </div>` : ''}
        ${finalAtkCrit > finalAtkSharp ? `
        <div class="stat-card cursor-pointer hover:border-[#e8d5a3] transition rounded shadow-md" onclick="toggleBreakdown('breakdown-crit')">
            <div class="flex justify-between items-center">
                <span class="stat-label">Crítico</span>
                <span class="text-[#4090f0] font-cinzel font-bold text-[20px] leading-none">${finalAtkCrit}</span>
            </div>
            <div class="stat-breakdown hidden" id="breakdown-crit">
                ${bd('Base', finalAtkSharp)}
                ${bd('Bônus crítico', critDmgBonus+'%')}
            </div>
        </div>` : ''}
        ${finalAtkQP > finalAtkSharp ? `
        <div class="stat-card cursor-pointer hover:border-[#e8d5a3] transition rounded shadow-md" onclick="toggleBreakdown('breakdown-qp')">
            <div class="flex justify-between items-center">
                <span class="stat-label">Quebra-parte</span>
                <span class="text-[#c090e0] font-cinzel font-bold text-[20px] leading-none">${finalAtkQP}</span>
            </div>
            <div class="stat-breakdown hidden" id="breakdown-qp">
                ${bd('Base', finalAtkSharp)}
                ${bdBonus('Dano em partes', bonusDmgQP, '%')}
            </div>
        </div>` : ''}
        ${finalAtkAir ? `
        <div class="stat-card hover:border-[#e8d5a3] transition rounded shadow-md">
            <div class="flex justify-between items-center">
                <span class="stat-label">No Ar</span>
                <span class="text-[#facc15] font-cinzel font-bold text-[20px] leading-none">${finalAtkAir}</span>
            </div>
        </div>` : ''}
        ${bonusAtord ? `
        <div class="stat-card hover:border-[#e8d5a3] transition rounded shadow-md">
            <div class="flex justify-between items-center">
                <span class="stat-label">Atordoamento</span>
                <span class="text-[#e8d5a3] font-cinzel font-bold text-[20px] leading-none">+${bonusAtord}%</span>
            </div>
        </div>` : ''}
        ${bonusLav ? `
        <div class="stat-card hover:border-[#e8d5a3] transition rounded shadow-md">
            <div class="flex justify-between items-center">
                <span class="stat-label">Exaustão</span>
                <span class="text-[#e8d5a3] font-cinzel font-bold text-[20px] leading-none">+${bonusLav}%</span>
            </div>
        </div>` : ''}
        ...
        <div class="stat-card hover:border-[#e8d5a3] transition rounded shadow-md">
            <div class="flex justify-between items-center">
                <span class="stat-label">Afin. P. Feridas</span>
                <span class="${affWeakspotColor} font-cinzel font-bold text-[20px] leading-none">${affWeakspot}%</span>
            </div>
        </div>` : ''}
        ${bonusAffFromSaqCrit ? `
        <div class="stat-card hover:border-[#e8d5a3] transition rounded shadow-md">
            <div class="flex justify-between items-center">
                <span class="stat-label">Afin. Imediato</span>
                <span class="text-[#4090f0] font-cinzel font-bold text-[20px] leading-none">+${bonusAffFromSaqCrit}%</span>
            </div>
        </div>` : ''}
        ${weaponElementHtml}`;
    ...
    Object.keys(sets).sort().forEach(setName=>{
        ...
        if(isActive) activeSetsHeader+=`<span class="bg-[#383124] border border-[#c9a84c] text-[#c9a84c] px-3 py-1 font-cinzel text-[13px] tracking-[1px] uppercase rounded shadow-md">${skillName}</span>`;
        setsDisplayHtml+=`
        <div class="p-3 bg-[#252016] border border-[#d9b85c]/40 mt-2 rounded shadow-sm group hover:border-[#d9b85c] transition">
            <div class="flex justify-between items-center">
                <span class="text-[13px] font-cinzel font-bold tracking-[1px] ${isActive?'text-[#e8d5a3]':'text-[#8a6e3a]'} group-hover:text-white uppercase">✦ ${skillName}</span>
                <span class="font-cinzel text-[13px] font-bold ${isActive?'text-[#e8d5a3]':'text-[#8a6e3a]'}">${count}/${req}</span>
            </div>
            <div class="text-[13px] ${isActive?'text-[#c9b07a]':'text-[#8a6e3a]'} mt-1.5 italic leading-tight group-hover:text-[#c9b07a]">${desc}</div>
        </div>`;
    });
...
(function(){
    ...
    // Forçar dark em selects no mobile
    document.querySelectorAll('select').forEach(s=>{
        s.style.setProperty('background','#252016','important');
        s.style.setProperty('color','#d9b85c','important');
    });
})();